  ...
  for (int i=ys;i<ye;i++){
    for (int t=xs-ys+i;t<xe;t++){
      if (fabs(powf(xo-t,2)+powf(yo-i,2))<=powf(r,2)){
        switch(quarter){
          case 1:OLED_DrawPoint(t,2*ys-i,color,layer);
            OLED_DrawPoint(xs+i-ys,ys+xs-t,color,layer);
            break;
          case 2:OLED_DrawPoint(2*xs-t,2*ys-i,color,layer);
            OLED_DrawPoint(xs+ys-i,ys+xs-t,color,layer);
            break;
          case 3:OLED_DrawPoint(2*xs-t,i,color,layer);
            OLED_DrawPoint(xs+ys-i,ys+t-xs,color,layer);
            break;
          case 4:OLED_DrawPoint(t,i,color,layer);
