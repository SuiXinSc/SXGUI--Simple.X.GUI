...
void Kernel_Infor(APP_PARAMETERS) {
  Graphics_ShowString(0, 24, "SXGUI v1.3", 16, WHITE);
}

void Graphics_Infor(APP_PARAMETERS) {
  Graphics_ShowString(0, 24, "图形库版本:v1.2", 16, WHITE);
}

void Driver_Infor(APP_PARAMETERS) {
  Graphics_ShowString(0, 24, "驱动版本:OLED v1.4.M", 16, WHITE);
}
...
void Controls(APP_PARAMETERS) {
  ...
  Progress_Bar(14, 24, 100, 6, 1, 0, 1, percent);
}

void Writer_Infor(APP_PARAMETERS) {
  static int tick;
  static bool CB = true;
  Graphics_ShowString(0, 0, "作者:岁心", 16, WHITE);
  if(Graphics_GetTick() - tick > 5000) {
    CB = !CB;
    tick = Graphics_GetTick();
  }
  if(CB) {
    Graphics_ShowString(0, 24, "CSDN: https://blog.csdn.net/m0_73677866", 12, WHITE);
    return;
  }
  Graphics_ShowString(0, 24, "Bilibili: https://space.bilibili.com/3494359452354953", 12, WHITE);
}

void PID_Infor(APP_PARAMETERS) {
  char tmp[16];
  sprintf(tmp, "Kp: %.3f", PID_Parameter.Kp);
  Graphics_ShowString(0, 0, tmp, 12, WHITE);
  sprintf(tmp, "Ki: %.3f", PID_Parameter.Ki);
  Graphics_ShowString(0, 12, tmp, 12, WHITE);
  sprintf(tmp, "Kd: %.3f", PID_Parameter.Kd);
  Graphics_ShowString(0, 24, tmp, 12, WHITE);
  sprintf(tmp, "Factor: %.3f", PID_Parameter.Factor);
  Graphics_ShowString(0, 36, tmp, 12, WHITE);
  sprintf(tmp, "MinTime: %dms", PID_Parameter.MinTime);
  Graphics_ShowString(0, 48, tmp, 12, WHITE);
}
...
int main(void) {
  ...
  while(1) {
    ...
    //主程序执行
    Scan_Key(Key);
    SXGUI_Main(16, BLACK, WHITE, FRAME_STYLE, Key);
    HAL_Delay(5);
  }
  /* USER CODE END 3 */
}
